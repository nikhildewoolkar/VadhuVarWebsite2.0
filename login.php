...
if($_SERVER["REQUEST_METHOD"] == "POST")
{
  ...
  {
    $sql = "SELECT id, fname, lname, username, emailid, password1, gender, photo, verified FROM register WHERE username = '". $username."' and verified <> '".$no."'";
    ...
    {            
      ...
      if(password_verify($password, $row["password1"]))
      {
        ...
        $sql = "UPDATE register set last_login = now() where username = '". $username."'";
        $conn->query($sql);
        ...
        if($photo == ""){
          if($gender == "Male"){
            $sql = "UPDATE register set photo = 'groom.png' where username = '". $username."'";
            $conn->query($sql);
          }
          else{
            $sql = "UPDATE register set photo = 'bride.png' where username = '". $username."'";
            $conn->query($sql);
          }
        }
 ...
 <div class="staticContainer paymentContainer">
   <!--  <div class="form-group clearfix">-->
    ...
    <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post">
      ...
      <div class="form-group clearfix">
        <label class="col-sm-4 labelControl"><a href="forgotpwd.php">Forgot Password ?</a></label>
        <div class="col-sm-8">					
        </div>
